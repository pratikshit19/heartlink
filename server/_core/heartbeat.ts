...
import { relationships, users } from "../../drizzle/schema";
...
/**
 * Run the heartbeat job to process scheduled notifications for all active relationships
 */
export async function heartbeatJob(): Promise<void> {
  console.log("[Heartbeat] Running periodic heartbeat job...");
  try {
    const database = await db.getDb();
    if (!database) {
      console.warn("[Heartbeat] Database not available, skipping heartbeat");
      return;
    }

    // Get all active relationships
    const activeRelationships = await database.select().from(relationships);
    console.log(`[Heartbeat] Found ${activeRelationships.length} relationships to process`);

    for (const relationship of activeRelationships) {
      // Find the user for this relationship to send notifications
      const userList = await database.select().from(users).where(eq(users.id, relationship.userId)).limit(1);
    ...
    console.log("[Heartbeat] Periodic heartbeat job completed successfully");
  } catch (error) {
    console.error("[Heartbeat] Error during heartbeat execution:", error);
  }
}
